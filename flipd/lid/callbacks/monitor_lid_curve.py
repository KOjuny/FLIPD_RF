...
class MonitorLIDCurve(MonitorMetrics):
    ...
    **LID estimation hyperparameters**:
    ...
    3. sweeping_arg:
        This is the hyperparameter that we are sweeping over. For example, in the FlipdEstimator, this could be
        the time t hyperparameter ranging from 0 to t_max (which is [0, 1]).
    4. sweeping_range:
        This is the range of the hyperparameter that we are sweeping over. For example, in the FlipdEstimator, this
        could be the time t hyperparameter ranging from 0 to t_max (which is [0, 1]).
    5. lid_estimation_args:
        All of the other arguments that are required for the LID estimator. For example, in the FlipdEstimator, this
        could be the number of hutchinson samples.
    ...
    **Examples**:
    ...
    1. Use the FlipdEstimator to monitor the LID curve. The best sweeping hyperparameter
    in this scenario is the time t hyperparameter ranging from 0 to t_max (which is [0, 1]).
    Once the model starts to fit the data manifold, the LID curve should plateau or kink at
    around the correct intrinsic dimensionality.

    2. Use the NormalBundleEstimator to monitor the LID curve. The best sweeping hyperparameter
    ...
    3. Use the FPRegressEstimator to monitor the LID curve. The best sweeping hyperparameter
    ...
    def __init__(
        ...
        Args:
            ...
            sweeping_arg (str):
                The hyperparameter that we are sweeping over, for example, with FLIPD estimators this is
                the time 't' and for NormalBundle estimators, this is the singular_value_threshold.
            sweeping_range (Iterable):
